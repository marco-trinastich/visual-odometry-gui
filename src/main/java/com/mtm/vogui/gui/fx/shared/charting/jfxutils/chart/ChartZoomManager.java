...
public class ChartZoomManager {
    @Getter
    private final BooleanProperty zooming = new SimpleBooleanProperty(false);
    ...
    public <X, Y> ChartZoomManager(Pane chartPane, Rectangle selectRect, XYChart<X, Y> chart) {
        ...
        handlerManager.addEventHandler(false, MouseEvent.MOUSE_PRESSED, this::onMousePressed);

        handlerManager.addEventHandler(false, MouseEvent.DRAG_DETECTED, this::onDragStart);
        ...
        handlerManager.addEventHandler(false, ZoomEvent.ANY, new TouchGestureZoomHandler());
    }
    ...
    private boolean doNotPassFilter(MouseEvent event) {
        if (mouseFilter != null) {
    ...
    private void onMousePressed(MouseEvent mouseEvent) {
        if (doNotPassFilter(mouseEvent)) {
            zoomMode = AxisConstraint.None;
            return;
        }
    ...
    private class TouchGestureZoomHandler implements EventHandler<ZoomEvent> {
        @Override
        public void handle(ZoomEvent event) {
            // If invalid event exit (ZOOM_FINISHED doesn't contain a partial zoomFactor)
            if (event.getEventType() != ZoomEvent.ZOOM_STARTED &&
                    event.getEventType() != ZoomEvent.ZOOM)
                return;

            //Are we zooming in or out, based on the gesture
            double zoomAmount = 1 - event.getZoomFactor();

            handleZoom(event.getX(), event.getY(), zoomAmount);
        }
    }
    ...
    private class MouseWheelZoomHandler implements EventHandler<ScrollEvent> {
        ...
        public void handle(ScrollEvent event) {
            ...
            double zoomAmount = 0.05 * direction;

            handleZoom(event.getX(), event.getY(), zoomAmount);
        }
    }

    private void handleZoom(double eventX, double eventY, double zoomAmount) {
        //If we are not allowing mouse wheel zooming exit
        if (!mouseWheelZoomAllowed.get())
            return;
        ...
        //If we are doing a zoom animation, stop it. Also of note is that we don't zoom the
    ...
    private static <T> DoubleProperty getLowerBoundProperty(Axis<T> axis) {
        return axis instanceof ValueAxis ?
                ...
                toDoubleProperty(axis,
                        Objects.requireNonNull(ChartZoomManager.getProperty(axis, "lowerBoundProperty")));
    }
    ...
    private static <T> DoubleProperty getUpperBoundProperty(Axis<T> axis) {
        return axis instanceof ValueAxis ?
                ...
                toDoubleProperty(axis,
                        Objects.requireNonNull(ChartZoomManager.getProperty(axis, "upperBoundProperty")));
    }
    ...
    private static <T> DoubleProperty toDoubleProperty(final Axis<T> axis, final Property<T> property) {
        ...
        doubleChangeListenerAry[0] = (observable, oldValue, newValue) -> {
            ...
            property.setValue(axis.toRealValue(newValue.doubleValue()));
