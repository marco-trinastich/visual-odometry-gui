...
public class ChartViewPort extends JViewport {
    ...
    public ChartViewPort(ChartState state, ChartStyle style, @NotNull JScrollPane scrollPane) {
    ...
    public void paint(Graphics g) {
        ...
        if (this.shouldShowLegend()) {
            ...
            for (int i = 0; i < this.state.series().size(); i++) {
                ...
                g.setColor(this.style.multipleColors() ? SwingUtils.generateColor(drawColor, i) : drawColor);
                // Draw chart name
    ...
    public void paintComponent(Graphics g) {
        ...
        if (this.shouldShowPermanentAxisNames()) {
            g.setColor(this.style.resolvedAxisNamesColor());

            if (this.isXAxisInViewport()) {
                String xAxisName = this.style.xAxisName().value();
                // Left: viewport end
                int left = this.getWidth() - (15 + SwingUtils.getStringDisplaySize(xAxisName));
                // Top: above X axis
            ...
            if (this.isYAxisInViewport()) {
                String yAxisName = this.style.yAxisName().value();
                // Left: left of Y axis
                int left = (this.state.originX() - this.horizontalBar.getValue()) -
                        (15 + SwingUtils.getStringDisplaySize(yAxisName));
                // Top: viewport top
                int top = 15;
    ...
    private boolean shouldShowLegend() {
        return this.state.hasPoints() &&
                this.style.showLegend();
    }

    private boolean shouldShowPermanentAxisNames() {
        return this.state.hasPoints() &&
                this.style.showAxis() &&
                this.style.showPermanentAxisNames();
    }
